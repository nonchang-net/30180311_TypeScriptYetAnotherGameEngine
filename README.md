...
- モンスター定義はマスターデータ管理を試みる。といっても大したことはしてなくて、Google Spread Sheet上で書いて、S3にデプロイするという仕組みを作成。
	- このフロー自体はこのリポジトリには入ってないので注意。


## やること

- `実際に自分のゲームで使うときは、git submoduleとして取り込む`形を想定。でもまだそんな段階でもない。
...
- 16:35
	- 実装できた。コミット。
	- 次やるべきことはなんだろう。
		- GameContextの保存・復旧を考えたい。つまりセーブ・ロードだ。
		- このコマンドはどこでも実行できて、ゲームオーバーでもセーブ・ロードができて、復旧できることが期待される。
		- 最初に設計しておくべき点だよな。検討しよう。
...
- 23:25
	- イベント回りを難しく考えすぎていた。イベントをシンプルに再設計。
	- YAGEフォルダも消した。もうSampleGameフォルダもいらない感じだな。
	- このコードはまだシンプルかつスケーラブルにできる気がしないでもない。うーん。

	- とりあえず新しい設計をコミットしておく。

- 00:31
	- 次にやるべきは、Ruleクラスの整理ということになりそう。
	- よく検討しよう。Rule適用というよりも、機能ベースでリプレイに着目してみるか。「再現可能な履歴」「Undo可能なもの」といったシステム要件が会った時に便利になる。
